...
Primero creamos un input para que el usuario registre un numero, una variable que sera utilizada en el if
para determinar si el numero es par, usando la operación de modulo.
...
Primero se crea 3 espacios para variables tipo float como se muestra en la primera parte del código,  luego creamos la variable suma que
se usara para determinar si el número 3 es mayor a esta usando la operación if y creando un enunciado usando print y agregando str() para 
...
Primero se crean las variables que el usuario ingresa,  y después se crean las operaciones de cada calculo,
para el promedio se hace de manera directa mientras que la mediana usa un modelo importado con import, se usa el sorted()
para ordenar los numeros de manera ascendente y descendente, y luego se usa max y min para los valores mayor y menor que se usaran para la potencia
...
Para el punto 9 se creo una lista de países con su capital y se uso if para según la variable ingresada, en este caso un país de america. se devolviera la 
respectiva capital.
